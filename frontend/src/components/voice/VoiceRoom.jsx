...
const VoiceRoom = () => {
  ...
  // Connect to LiveKit room and start voice session
  const handleStart = async () => {
    try {
      setIsConnecting(true);
      setError("");
      const data = await getLiveKitToken();
      // LiveKit connection logic will go here in next step
      setIsConnected(true);
      addMessage("Connected! Eve is listening...", false);
    } catch (err) {
  ...
  const handleQuickAction = (label) => {
    ...
    setTimeout(() => {
      addMessage(`Sure! I can help you with ${label}. Please start the voice call or type your question.`, false);
    }, 800);
  };
  ...
  const handleTypedSubmit = (e) => {
    ...
    setTypedMessage("");
    setTimeout(() => {
      addMessage("Please start the voice call so Eve can assist you better.", false);
    }, 800);
  };
